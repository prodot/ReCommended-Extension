...
public sealed class Int128Info() : SignedIntegerInfo<Int128Info.Int128>(ClrTypeNames.Int128)
{
    /// <remarks>
    ...
    /// </remarks>
    public readonly new record struct Int128 // todo: remove when available (used only for testing)
    {
    ...
    static readonly int maxValueStringLength = Int128.MaxValue.ToString().Length;

    internal override TypeCode? TypeCode => null;

    internal override int? MaxValueStringLength => maxValueStringLength;

    internal override Int128? TryGetConstant(ICSharpExpression? expression, out bool implicitlyConverted)
    {
    ...
    internal override string CastConstant(ICSharpExpression constant, bool implicitlyConverted) => constant.Cast("Int128").GetText();

    internal override string Cast(ICSharpExpression expression) => expression.Cast("Int128").GetText();

    internal override string CastZero(CSharpLanguageLevel languageLevel) => "(Int128)0";

    internal override bool AreEqual(Int128 x, Int128 y) => x == y;

    internal override bool IsZero(Int128 value) => value == 0;

    internal override bool AreMinMaxValues(Int128 min, Int128 max) => (min, max) == (Int128.MinValue, Int128.MaxValue);
}