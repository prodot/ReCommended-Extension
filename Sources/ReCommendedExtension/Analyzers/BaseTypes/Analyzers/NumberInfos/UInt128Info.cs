...
public sealed class UInt128Info() : UnsignedIntegerInfo<UInt128Info.UInt128>(ClrTypeNames.UInt128)
{
    /// <remarks>
    ...
    /// </remarks>
    public readonly new record struct UInt128 // todo: remove when available (used only for testing)
    {
    ...
    internal override UInt128? TryGetConstant(ICSharpExpression? expression, out bool implicitlyConverted)
    {
    ...
    internal override string CastConstant(ICSharpExpression constant, bool implicitlyConverted) => constant.Cast("UInt128").GetText();

    internal override string Cast(ICSharpExpression expression) => expression.Cast("UInt128").GetText();

    internal override string CastZero(CSharpLanguageLevel languageLevel) => "(UInt128)0";

    internal override bool AreEqual(UInt128 x, UInt128 y) => x == y;

    internal override bool IsZero(UInt128 value) => value == 0;

    internal override bool AreMinMaxValues(UInt128 min, UInt128 max) => (min, max) == (UInt128.MinValue, UInt128.MaxValue);
}