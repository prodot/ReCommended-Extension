...
public sealed class HalfInfo() : FloatingPointNumberInfo<HalfInfo.Half>(ClrTypeNames.Half)
{
    /// <remarks>
    ...
    /// </remarks>
    public readonly new record struct Half // todo: remove when available (used only for testing)
    {
    ...
    internal override string? NanConstant => null;

    internal override Half? TryGetConstant(ICSharpExpression? expression, out bool implicitlyConverted)
    {
    ...
    internal override string Cast(ICSharpExpression expression) => expression.Cast("Half").GetText();
