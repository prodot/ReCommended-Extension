...
public sealed class UnthrowableExceptionAnalyzer : ElementProblemAnalyzer<ICSharpTreeNode>
{
    static readonly Dictionary<string, string> unthrowableExceptions = new(StringComparer.Ordinal)
    {
        ...
        { "System.ExecutionEngineException", "The exception should only be thrown by the CLR." },
        ...
        { "System.Runtime.InteropServices.COMException", "The exception should only be thrown by the CLR." },
    ...
    protected override void Run(ICSharpTreeNode element, ElementProblemAnalyzerData data, IHighlightingConsumer consumer)
    {
        ...
        if (exception is { } && unthrowableExceptions.TryGetValue(exception.GetExpressionType().ToString(), out var reason))
        {
