...
internal sealed record RedundantMethodInvocation : Inspection
{
    ...
    {
        Condition = (qualifier, args) => qualifier.AsBooleanConstant == null && args is [{ Value.AsBooleanConstant: true }],
        Message = methodName => $"Calling '{methodName}' with true is redundant.",
    };

    public static RedundantMethodInvocation WithNull { get; } = new()
    {
        Condition = (_, args) => args is [{ Value.IsDefaultValueOrNull: true }],
    ...
    {
        Condition = (_, args) => args is [{ Value.AsInt32Constant: 0 }, ..],
        Message = methodName => $"Calling '{methodName}' with 0 is redundant.",
    };

    public static RedundantMethodInvocation WithInt64Zero { get; } = new()
    {
        Condition = (_, args) => args is [{ Value.AsInt64Constant: 0 }], Message = methodName => $"Calling '{methodName}' with 0 is redundant.",
    };

    public static RedundantMethodInvocation WithEmptyString { get; } = new()
    {
        Condition = (_, args) => args is [{ Value.AsStringConstant: "" }],
    ...
    {
        Condition = (_, args) => args is [{ Value.AsCollectionCreation.Count : 0 }],
    ...
    {
        Condition = (_, args) => args is [_, { Value.IsDefaultValueOrNull: true }],
        Message = methodName => $"Calling '{methodName}' with null is redundant.",
    };

    public static RedundantMethodInvocation WithEmptyCollectionInArg1 { get; } = new()
    {
        Condition = (_, args) => args is [_, { Value.AsCollectionCreation.Count: 0 }],
        Message = methodName => $"Calling '{methodName}' with an empty collection is redundant.",
    };

    public static RedundantMethodInvocation WithEmptyCollectionInParamsArg1 { get; } = new()
    {
        Condition = (_, args) => args is [_] or [_, { Value.AsCollectionCreation.Count: 0 }],
        Message = methodName => $"Calling '{methodName}' with an empty collection is redundant.",
    };

    public static RedundantMethodInvocation WithRepeatCountZeroInArg1 { get; } = new()
    {
        Condition = (_, args) => args is [_, { Value.AsInt32Constant: 0 }],
    ...
    {
        Condition = (_, args) => args is [{ Value.AsCharConstant: { } c0 }, { Value.AsCharConstant: { } c1 }] && c0 == c1,
    ...
    {
        Condition = (_, args) => args is [{ Value.AsStringConstant: { } s0 and not "" }, { Value.AsStringConstant: { } s1 }] && s0 == s1,
    ...
    {
        Condition = (_, args) => args is
            [
                { Value.AsStringConstant: { } s0 and not "" },
                { Value.AsStringConstant: { } s1 },
                { Value.AsStringComparisonConstant: StringComparison.Ordinal },
            ]
            && s0 == s1,
        Message = methodName => $"Calling '{methodName}' with identical values is redundant.",
    };
    ...
    {
        Condition = (_, args) => args is [{ Value.IsDefaultValueOrNull: true }, { Value.AsInt32Constant: 0 }, { Value.AsInt32Constant: 0 }],
    ...
    {
        Condition = (_, args) => args is [{ Value.AsCollectionCreation.Count: 0 }, { Value.AsInt32Constant: 0 }, { Value.AsInt32Constant: 0 }],
    ...
    {
        Condition = (_, args) => args is [_, _, { Value.AsInt32Constant: 0 }],
