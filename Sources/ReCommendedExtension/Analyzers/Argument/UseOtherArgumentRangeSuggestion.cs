...
public sealed class UseOtherArgumentRangeSuggestion(string message) : MultipleHighlightings(message)
{
    const string SeverityId = "UseOtherArgumentRange";

    public required IReadOnlyList<ArgumentReplacement> ArgumentReplacements { get; init; }

    public required ICSharpArgument? RedundantArgument { get; init; }

    [QuickFix]
    public sealed class Fix(UseOtherArgumentRangeSuggestion highlighting) : QuickFixBase
    {
            ...
            get
            {
                ...
                builder.Append(
                    $"Replace with '{string
                        ...
                        .TrimToSingleLineWithMaxLength(120)}', respectively");
