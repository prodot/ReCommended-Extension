...
{
    extension([NotNullWhen(true)] ICSharpTreeNode? element)
    {
        [Pure]
        public bool IsDefaultValueOf(IType type)
        {
            switch (element)
            {
    ...
    extension(ICSharpTreeNode treeNode)
    {
        [Pure]
        public CSharpCompilerNullableInspector? TryGetNullableInspector(
