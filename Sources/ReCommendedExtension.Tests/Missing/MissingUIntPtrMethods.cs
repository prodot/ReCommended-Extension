...
{
    extension(nuint source) // todo: consider implementing IFormattable
    {
        [Pure]
        ...
        public static nuint RotateLeft(nuint value, int rotateAmount)
            => UIntPtr.Size switch
            {
                8 => (nuint)ulong.RotateLeft(value, rotateAmount),
                4 => uint.RotateLeft((uint)value, rotateAmount),
        ...
        public static nuint RotateRight(nuint value, int rotateAmount)
            => UIntPtr.Size switch
            {
                8 => (nuint)ulong.RotateRight(value, rotateAmount),
                4 => uint.RotateRight((uint)value, rotateAmount),

                _ => throw new PlatformNotSupportedException(),
            };

        [Pure]
        public string ToString(string? format) => source.ToString(format, NumberFormatInfo.CurrentInfo);

        [Pure]
        public string ToString(IFormatProvider? provider) => source.ToString(null, provider);

        [Pure]
        public string ToString(string? format, IFormatProvider? provider) => source.ToString();
        ...
        [Pure]
        public static bool TryParse(ReadOnlySpan<byte> utf8Text, out nuint result) => TryParse(Encoding.UTF8.GetString([..utf8Text]), out result);
    }
}