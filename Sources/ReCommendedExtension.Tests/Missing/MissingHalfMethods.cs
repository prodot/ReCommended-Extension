...
internal static class MissingHalfMethods
{
    extension(Half)
    {
        ...
        [Pure]
        public static Half Round(Half x) => (Half)float.Round((float)x);

        [Pure]
        public static Half Round(Half x, [ValueRange(0, 6)] int digits) => (Half)float.Round((float)x, digits);

        [Pure]
        public static Half Round(Half x, MidpointRounding mode) => (Half)float.Round((float)x, mode);

        [Pure]
        public static Half Round(Half x, [ValueRange(0, 6)] int digits, MidpointRounding mode) => (Half)float.Round((float)x, digits, mode);
