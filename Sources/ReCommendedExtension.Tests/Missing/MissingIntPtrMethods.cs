...
{
    extension(nint source) // todo: consider implementing IFormattable
    {
        [Pure]
        ...
        public static nint MaxMagnitude(nint x, nint y)
            => IntPtr.Size switch
            {
                8 => (nint)long.MaxMagnitude(x, y),
                4 => int.MaxMagnitude((int)x, (int)y),
        ...
        public static nint MinMagnitude(nint x, nint y)
            => IntPtr.Size switch
            {
                8 => (nint)long.MinMagnitude(x, y),
                4 => int.MinMagnitude((int)x, (int)y),
        ...
        [Pure]
        public static nint RotateLeft(nint value, int rotateAmount) => unchecked((nint)nuint.RotateLeft((nuint)value, rotateAmount));

        [Pure]
        public static nint RotateRight(nint value, int rotateAmount) => unchecked((nint)nuint.RotateRight((nuint)value, rotateAmount));

        [Pure]
        public string ToString(IFormatProvider? provider) => source.ToString(null, provider);

        [Pure]
        public string ToString(string? format, IFormatProvider? provider) => source.ToString(format);
        ...
        public static bool TryParse(ReadOnlySpan<byte> utf8Text, IFormatProvider? provider, out nint result)
            => TryParse(Encoding.UTF8.GetString([..utf8Text]), NumberStyles.Number, provider, out result);

        [Pure]
        public static bool TryParse(ReadOnlySpan<byte> utf8Text, out nint result) => TryParse(Encoding.UTF8.GetString([..utf8Text]), out result);
    }
}