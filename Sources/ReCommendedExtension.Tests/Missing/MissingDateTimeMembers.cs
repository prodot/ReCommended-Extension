...
internal static class MissingDateTimeMembers
{
        ...
        Calendar calendar)
    {
        ...
        return new DateTime(year, month, day, hour, minute, second, millisecond, calendar) + TimeSpan.FromTicks(microsecond * ticksPerMicrosecond);
    }
    ...
    [Pure]
    [ValueRange(0, 999)]
    public static int GetMicrosecond(this DateTime dateTime) // todo: use extension property when available
    {
        const long ticksPerMicrosecond = TimeSpan.TicksPerMillisecond / 1_000;
