...
namespace Test
{
    public class Methods
    {
        ...
        public void NoDetection(string text, string? textNullable, int totalWidth, char c, string s, int startIndex, int count, StringComparison comparisonType)
        {
            ...
            var resultH1 = text.EndsWith(s, comparisonType);
            var resultH2 = textNullable?.EndsWith('c');
            var resultH3 = textNullable?.EndsWith(c);
            var resultH4 = textNullable?.EndsWith("c", StringComparison.Ordinal);
            var resultH5 = textNullable?.EndsWith("c", StringComparison.OrdinalIgnoreCase);

            var resultI1 = text.StartsWith(s, comparisonType);
            var resultI2 = textNullable?.StartsWith('c');
            var resultI3 = textNullable?.StartsWith(c);
            var resultI4 = textNullable?.StartsWith("c", StringComparison.Ordinal);
            var resultI5 = textNullable?.StartsWith("c", StringComparison.OrdinalIgnoreCase);
